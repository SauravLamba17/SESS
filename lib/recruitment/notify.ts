 ...
 *     Employee record.
 *   • `User` rows are only ever created by prisma/seed-test-data.ts — the
 *     Phase 5 onboarding flow creates an Employee and NO User row.
 *   • In the live database today: 1 HR user, and it DOES have an Employee
 *     record. Zero SUPER_ADMIN `User` rows exist at all — the real Super Admin
 *     authenticates through Clerk with no DB row.
 *
 * So "every HR user has an Employee record" is TRUE of the current data but is
 * NOT structurally guaranteed. Rather than restructure Notification (adding a
 * nullable `recipientRole` would make `employeeId` nullable and weaken the
 * guarantee every existing PAYSLIP_READY row relies on), this resolves
 * recipients through the existing User→Employee link and makes the
 * zero-recipient case LOUD instead of silent. If HR users ever exist without
 * Employee records, the count returned here goes to 0 and the caller logs it —
 * which is the signal to add `recipientRole` for real.
 */
