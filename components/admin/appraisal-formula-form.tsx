...
import { ZERO_WEIGHTS, type ResolvedFormula } from "@/lib/appraisal/formula-config";
...
const ZERO = ZERO_WEIGHTS as Record<Key, number>;

/**
 * The Super Admin formula editor.
 *
 * The INITIAL state arrives as props from the server component — this no longer
 * fetches its own first paint. Switching department is a real user interaction,
 * so that still goes over the API, against the same resolveFormula() the server
 * used, which is why both paths agree.
 */
export function AppraisalFormulaForm({ initial }: { initial: ResolvedFormula }) {
  const [department, setDepartment] = useState(initial.department ?? "");
  const [weights, setWeights] = useState<Record<Key, number>>({
    ...ZERO,
    ...initial.weights,
  });
  ...
  /** Department changed by the user — refetch that department's formula. */
